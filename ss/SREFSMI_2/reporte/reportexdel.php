...
$a=mysql_query("SELECT	catunimed13.delegado AS delegado, catunimed13.jpm AS jpm FROM catunimed13 where catunimed13.del = '$del' GROUP BY catunimed13.del",$conn2) or die(mysql_error());
$f=mysql_fetch_array($a);

require("../conexion.php");

$query=mysql_query("SELECT del, delegoumae FROM cat_clues_ss WHERE del='$del'");
...
{	
	$pdf->SetXY(20,60);
	$pdf->MultiCell(246,4,utf8_decode('"'.$uno.' no reporta intervenciones Excedentes/Faltantes para el año 2020"'), 0, "C",0,0,0,0);
}
else if($um>1)
{
	if($umconr>1)
...
SELECT
	...
	pacientes
	FROM
requerimiento_ss a LEFT JOIN cat_clues_ss b ON a.clp=b.clp2
WHERE
	b.del = '$del' AND
	excedentea>0
GROUP BY
