...
CLUES: <?php echo $clues;?> / Unidad Médica: <?php echo $nom_sec;?><br><br>
</div>
...
haciendo clic izquierdo sobre el nombre del mismo.</div><br>
...
esp_tron_o_serv*/
ORDER BY
no_grup_clas, gpo_serva, gpo_serv");

while($row=mysql_fetch_array($res))
{
	++$i;			
	$iduno[$i]=$row["gpo_serva"];
	$iddos[$i]=$row["gpo_serv"];
...
?>			
    ...
    <td rowspan="<?php echo $nrowxiduno[$i];?>"><div align="center"><input type="text" name="<?php echo "servicio".$i;?>" id="<?php echo "servicio".$i;?>" value="<?php echo $nombreuno[$i];?>" style="display:none"><?php echo $nombreuno[$i];?></div></td>            
	<?php 
	for($b=1; $b<=$nrowxiduno[$i]; $b++)
	{ 
	++$d; ?>
		<td onMouseOver="pintar(this,'#FFFFcc')" onMouseOut="pintar(this,'#FFFFFF')">
        <input type="text" name="<?php echo "grupo".$d;?>" id="<?php echo "grupo".$d;?>" value="<?php echo $nombredos[$d];?>" style="display:none">        
        <div align="left"><span onClick="xyz(<?php echo $i;?>,<?php echo $d;?>)"><?php echo $d." ".$nombredos[$d];?></span></div>
        </td>
        
		<td onMouseOver="pintar(this,'#FFFFcc')" onMouseOut="pintar(this,'#FFFFFF')"><div align="center"><?php if($eof[$d]>0) echo "<strong>SI</strong>";?></div></td>
		<?php  ?>
        </tr>	
       <?php }}?>
      </tbody>
