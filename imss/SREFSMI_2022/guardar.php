...
$query=mysql_query("SELECT tipo, numero, localidad FROM cat_clues WHERE clp='$clp'");
$datos=mysql_fetch_array($query); $tipo=$datos["tipo"]; $numero=$datos["numero"]; $localidad=$datos["localidad"];

if($cerrar==1)
{
	mysql_query("UPDATE cat_clues SET cerrado='$cerrar' WHERE del='$del'");
}

else if($excedenteofaltante==0)
{
	mysql_query("UPDATE cat_clues SET excedenteofaltante='$excedenteofaltante' WHERE del='$del'"); //$excedenteofaltante=0
	//para borrar todo lo que haya capturado la unidad médica
	mysql_query("INSERT INTO requerimiento_historico(clp, tipo, numero, localidad, clues, grupo, especialidad, producto, excedentea, excedenteh, faltantea, faltanteh, causa, fecsis, ipconfig, pacientes) 
	SELECT clp, tipo, numero, localidad, clues, grupo, especialidad, producto, excedentea, excedenteh, faltantea, faltanteh, causa, fecsis, ipconfig, pacientes FROM requerimiento WHERE del='$del'");
	mysql_query("DELETE FROM requerimiento WHERE del='$del'");
}
else if($excedenteofaltante==1)
{
	mysql_query("UPDATE cat_clues SET excedenteofaltante='1' WHERE del='$del'");
	
	$exister=mysql_query("SELECT * FROM requerimiento WHERE clp='$clp' AND grupo='$grupo' AND especialidad='$especialidad'");
	if(mysql_num_rows($exister)>0)
	{
		mysql_query("INSERT INTO requerimiento_historico(del, delegoumae, clp, tipo, numero, localidad, clues, grupo, especialidad, producto, excedentea, excedenteh, faltantea, faltanteh, causa, fecsis, ipconfig, pacientes) 
	SELECT del, delegoumae, clp, tipo, numero, localidad, clues, grupo, especialidad, producto, excedentea, excedenteh, faltantea, faltanteh, causa, fecsis, ipconfig, pacientes FROM requerimiento WHERE clp='$clp' AND grupo='$grupo'");
		mysql_query("DELETE FROM requerimiento WHERE clp='$clp' AND grupo='$grupo' AND especialidad='$especialidad'");
	}
