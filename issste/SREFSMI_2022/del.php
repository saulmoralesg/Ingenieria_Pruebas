...
<tr>
<td><a href="Manual/Guia_ayuda_2021_issste_2022.pdf"><img src="Imagenes/guia.jpg" border="0" width="37" alt="Guía de Ayuda"/></a><br><strong>Guía de Ayuda</strong></td>
</table>
<br>
<!--
...
<div align="center" style="background-color:#FFF; border:0; color:#000; font-size:14px; font-weight:bold;">  
  ...
  <br>
    Captura del periodo 2022</div>
<div align="center" style="background-color:#FFF; border:0; color:#000; font-size:14px; font-weight:bold">  
	<br>
Paso 1.- Seleccionar la Entidad Federativa a la que pertenece,<br>
haciendo clic izquierdo sobre el nombre de la misma.</div>
<div>
...
CASE
WHEN MAX(excedenteofaltante)>0 && MAX(cantidad)>0 THEN 'del_conexcedenteofaltante'
WHEN MAX(excedenteofaltante)>0 && MAX(cantidad) IS NULL THEN 'del_sinregistro'
WHEN MAX(excedenteofaltante)=0 THEN 'del_sinnecesidad'
WHEN MAX(excedenteofaltante) IS NULL THEN 'del_sinregistro' END AS status,
MAX(cerrado) AS cerrado,
CASE
WHEN del='5T' THEN 33 
WHEN del='5E' THEN 34
...
WHEN del='5N' THEN 50
WHEN del='5L' THEN 51
WHEN del='5C' THEN 52 ELSE del END AS delfororder
FROM
(
SELECT
del,
delegoumae,
COUNT(DISTINCT(clp2)) AS unidades,
sum(excedenteofaltante) AS excedenteofaltante,
MAX(cerrado) AS cerrado,
NULL AS cantidad
FROM
cat_clues_issste_2022 a
...
NULL AS cerrado,
sum(excedentea+excedenteh+faltantea+faltanteh) AS cantidad
FROM
requerimiento_issste_2022
GROUP BY
del
) c
GROUP BY
