...
<td><a href="Manual/Guia_ayuda_2021_issste_2022_2022.pdf"><img src="Imagenes/guia.jpg" border="0" width="37" alt="Guía de Ayuda"/></a><br><strong>Guía de Ayuda</strong></td>
...
-->
<div align="center" style="background-color:#FFF; border:0; color:#000; font-size:16px; font-weight:bold"><br>
...
FROM
cat_clues_issste_2022  a LEFT JOIN requerimiento_issste_2022  b ON a.clp2=b.clp
WHERE
a.del='$del'
GROUP BY
a.clp2
...
while($row=mysql_fetch_array($res))
{
	...
	$clp[$a]=$row["clp2"];
    $clues[$a]= $row["clues"];
	$nom_sec[$a]= $row["nom_sec"];
	$nom_imss[$a]=$row["tipo"]." ".$row["numero"].", ".$row["localidad"];
	$excedenteofaltante[$a]=$row["excedenteofaltante"];
	if($excedenteofaltante[$a]=="")
	$semaforo[$a]='<img src="Imagenes/rojo.png" alt="" width="25" height="25">';
	else
	$semaforo[$a]='<img src="Imagenes/verde.png" alt="" width="25" height="25">';
	$nivel[$a]=$row["localidad"];
}
for($i=1; $i<=$a; $i++)
{
	if($i==1 || $nivel[$i]!=$nivel[$i-1])
	{?>
    <tr>
    <td colspan="4" style="background-color:#EAF2D3; font-weight:bold"><?php echo $nivel[$i];?></td>
    </tr>
	<?php } ?>
          ...
          <tr onMouseOver="pintar(this,'#FFFFcc')" onMouseOut="pintar(this,'#FFFFFF')">
			...
			<input type="text" id="<?php echo "clp".$i;?>" name="<?php echo "clp".$i;?>" value="<?php echo $clp[$i];?>" style="display:none">
			<input type="text" id="<?php echo "clues".$i;?>" name="<?php echo "clues".$i;?>" value="<?php echo $clues[$i];?>" style="display:none">
			<input type="text" id="<?php echo "nom_sec".$i;?>" name="<?php echo "nom_sec".$i;?>" value="<?php echo $nom_sec[$i];?>" style="display:none">
			<input type="text" id="<?php echo "nom_imss".$i;?>" name="<?php echo "nom_imss".$i;?>" value="<?php echo $nom_imss[$i];?>" style="display:none">			</td>
            <td <?php //if($i % 2 != 0) echo 'style="background-color:#EAF2D3"';?> align="left"><div align="right"><span onClick="ums(<?php echo $i;?>)"><?php echo $clues[$i];?></span></div></td>			
            <td <?php //if($i % 2 != 0) echo 'style="background-color:#EAF2D3"';?> align="left"><div align="left"><?php if($cerrado!=1) {?><span onClick="ums(<?php echo $i;?>)"><?php echo $nom_imss[$i];?></span><?php } else echo $nom_imss[$i];?></div></td>
            <td <?php //if($i % 2 != 0) echo 'style="background-color:#EAF2D3"';?> align="Right"><div align="center"><strong><?php if($excedenteofaltante[$i]==1) echo "SI"; else if($excedenteofaltante[$i]==NULL) echo ""; else if($excedenteofaltante[$i]==0) echo "NO"; ?></strong></div></td>
